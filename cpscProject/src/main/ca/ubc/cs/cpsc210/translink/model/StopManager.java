...
public class StopManager implements Iterable<Stop> {
    ...
    public Stop getStopWithNumber(int number) {
        if (stopMap.containsKey(number)) {
            ...
            Stop stop = new Stop(number, "", new LatLon(49.201497, -122.913699));
    ...
    public Stop findNearestTo(LatLon pt) {
        Stop nearest = null;
        double nearestDistSoFar = RADIUS;

        for (Stop stop : stopMap.values()) {
            double dist = SphericalGeometry.distanceBetween(pt, stop.getLocn());
            if((dist < RADIUS) && (dist < nearestDistSoFar)){
                nearestDistSoFar = dist;
                nearest = stop;
            }
        }
